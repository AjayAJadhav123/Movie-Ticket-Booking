...
class RecommendationEngine:
    ...
    def get_user_profile_vector(self, user_movies: List[str], all_movies_df: pd.DataFrame) -> np.ndarray:
        ...
        # MongoDB stores movie IDs as ObjectIds, while the user profile stores
        # their string form. Use the dataframe already fetched from MongoDB
        # instead of querying with an incompatible string ID for every movie.
        watched_ids = {str(movie_id) for movie_id in user_movies}
        watched_movies = all_movies_df[
            all_movies_df["movie_id"].astype(str).isin(watched_ids)
        ]
        user_vectors = [
            self.create_feature_vector(movie, len(self.genre_map))
