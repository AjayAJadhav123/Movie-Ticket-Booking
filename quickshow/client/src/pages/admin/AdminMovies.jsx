  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-[#0f172a] p-6 rounded-xl shadow-sm border border-slate-800/50">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <Film className="text-red-500" /> Manage Movies
          </h1>
          <p className="text-slate-500 text-sm mt-1">Import and manage movies synced from TMDB</p>
        </div>
        <button
          onClick={() => setShowTMDBModal(true)}
          className="bg-red-600 text-white px-5 py-2.5 rounded-xl font-semibold hover:bg-red-700 transition flex items-center gap-2"
        >
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          {(searchTerm || selectedGenre) && (
            <button
              ...
              className="px-4 py-2.5 text-slate-400 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition font-medium"
            >
              Clear Filters
            </button>
          )}
      ...
      {loading ? (
        ...
        <div className="bg-[#0f172a] rounded-xl shadow-sm border border-slate-800/50 p-12 text-center">
          <AlertCircle size={48} className="mx-auto text-slate-300 mb-4" />
          <h3 className="text-lg font-semibold text-slate-400 mb-2">No Movies Found</h3>
                  ...
                  <tr key={movie._id} className="hover:bg-[#09090b] transition">
                    <td className="px-6 py-3">
                      {movie.poster_path ? (
                        ...
                        <div className="h-16 w-12 bg-slate-700 rounded-md flex items-center justify-center text-[10px] text-slate-500 text-center">No Image</div>
                      )}
                    </td>
                    <td className="px-6 py-3 font-semibold text-slate-200">{movie.title}</td>
                    <td className="px-6 py-3 text-slate-400">{movie.release_date ? new Date(movie.release_date).toLocaleDateString() : 'N/A'}</td>
                    <td className="px-6 py-3 text-slate-400 font-medium">★ {movie.rating?.toFixed(1) || '0.0'}</td>
                    <td className="px-6 py-3 hidden md:table-cell">
                      <div className="flex flex-wrap gap-1">
                        {movie.genres?.slice(0, 2).map((genre) => (
                          <span key={genre} className="bg-red-500/10 text-red-400 border border-indigo-100 px-2 py-0.5 rounded-md text-xs">{genre}</span>
                        ))}
      ...
      {showTMDBModal && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-[#0f172a] rounded-xl shadow-xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
            <div className="p-6 border-b border-slate-800/50 flex justify-between items-center bg-[#09090b]">
              <h2 className="text-xl font-bold text-white">Import from TMDB</h2>
              <button onClick={() => setShowTMDBModal(false)} className="text-slate-500 hover:bg-slate-700 p-2 rounded-full transition">
                <X size={20} />
              </button>
            </div>
              ...
              {tmdbApiKeyMissing ? (
                ...
                <div className="flex gap-3">
                  <input
                    ...
                    className="flex-1 px-4 py-2.5 rounded-xl border border-slate-700 focus:outline-none focus:border-red-500/50 focus:ring-2 focus:ring-red-500/20"
                  />
                  <button
                    onClick={searchTMDB}
                    disabled={tmdbLoading}
                    className="bg-red-600 text-white px-6 py-2.5 rounded-xl font-semibold hover:bg-red-700 disabled:opacity-50 transition flex items-center gap-2"
                  >
            ...
            <div className="flex-1 overflow-y-auto p-6 pt-0 bg-[#09090b]">
              {tmdbResults.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-4">
                  {tmdbResults.map((movie) => (
                    <div key={movie.id} className="bg-[#0f172a] border border-slate-800/50 rounded-xl overflow-hidden hover:shadow-md transition group">
                      {movie.poster_path ? (
                        <img src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt={movie.title} className="w-full h-48 object-cover" />
                      ) : (
                        <div className="w-full h-48 bg-slate-800 flex items-center justify-center text-slate-400">No Image</div>
                      )}
                      <div className="p-4">
                        <h4 className="font-bold text-white truncate" title={movie.title}>{movie.title}</h4>
